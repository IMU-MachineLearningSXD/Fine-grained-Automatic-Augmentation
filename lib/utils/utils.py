...
class strLabelConverter(object):
    ...
    def __init__(self, alphabet, ignore_case=False):
        ...
        #self.alphabet = alphabet.append('^')  # for `-1` index
        self.alphabet = alphabet
        

        self.dict = {}   # 一个字典 输出大概是 {'m':1,'T':2, .....}
    ...
    def encode(self, text):
        ...
        # text表示一个batch下的label：如['long', 'of', ....]

        length = []
        result = []
        decode_flag = True if type(text[0])==bytes else False   # 这个得改

        for item in text:     # 逐个访问text下的单词
            if decode_flag:   # 如果不是abc类型的话，解码成abc类型，一般不走这一条
                item = item.decode('utf-8', 'strict')  # 调用的是python自带的，不是下面的decode
            # item = item.split('|')
            length.append(len(item))  # 存入item的长度
            for char in item:
...
def get_char_dict(path):  # 这个函数都没有用到
